...
def index():
    ...
    # =========================================================
    # 2. GRÁFICO MODALIDADE (LÓGICA BLINDADA)
    # =========================================================
    qtd_presencial = 20
    qtd_distancia = 3
    ...
    fig_ensino.update_layout(title='<b>Modalidade</b>', **LAYOUT_KWARGS, margin=dict(t=50, b=20, l=10, r=10), height=380, title_x=0.5, showlegend=True, legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="center", x=0.5))
    html_ensino = fig_ensino.to_html(full_html=False, include_plotlyjs=False)

    # =========================================================
    # 3. GRÁFICO INSTITUIÇÃO (LÓGICA BLINDADA)
    # =========================================================
    qtd_privada = 17
    qtd_publica = 6
    ...
    fig_inst.update_layout(title='<b>Instituição</b>', **LAYOUT_KWARGS, margin=dict(t=50, b=20, l=10, r=10), height=380, title_x=0.5, showlegend=True, legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="center", x=0.5))
    ...
    def criar_rosca(dados, coluna, titulo, paleta_cores):
        ...
        fig = px.pie(contagem, names=coluna, values="Quantidade", hole=0.6, title=f"<b>{titulo}</b>", color_discrete_sequence=paleta_cores)
        fig.update_traces(textposition='inside', textinfo='percent')
        fig.update_layout(**LAYOUT_KWARGS, margin=dict(t=50, b=20, l=10, r=10), height=380, title_x=0.5, showlegend=True, legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="center", x=0.5))
        return fig.to_html(full_html=False, include_plotlyjs=False)

    cores_programa = ['#8b5cf6', '#c4b5fd', '#ede9fe']
